...
function convertV1toV2(src, dst) {
  const serializedBytes = import_aptos.BCS.bcsToBytes(src);
  const deserializerV2 = new import_ts_sdk.Deserializer(serializedBytes);
  return dst.deserialize(deserializerV2);
}
function convertV2toV1(src, dst) {
  const serializedBytes = src.bcsToBytes();
  const deserializerV1 = new import_aptos.BCS.Deserializer(serializedBytes);
...
async function generateV1TransactionPayload(payloadData, network) {
  ...
  const newPayload = await (0, import_ts_sdk.generateTransactionPayload)({
    ...payloadData,
    aptosConfig
  });
  return convertV2toV1(newPayload, import_aptos.TxnBuilderTypes.TransactionPayload);
}
...
var PetraWallet = class {
  ...
  async signAndSubmitTransaction(payloadV1OrGenerateTxnInput, optionsV1) {
    ...
    if ("data" in payloadV1OrGenerateTxnInput) {
      ...
      const options = {
        ...
        sender: generateTxnInput.sender ? import_ts_sdk2.AccountAddress.from(generateTxnInput.sender).toString() : void 0,
        ...generateTxnInput.options
      };
      if ((0, import_wallet_adapter_core.areBCSArguments)(generateTxnInput.data.functionArguments)) {
  ...
  async signTransaction(transactionOrPayload, optionsOrAsFeePayer) {
    ...
    if ("rawTransaction" in transactionOrPayload) {
      ...
      const rawTxnV1 = convertV2toV1(transaction.rawTransaction, import_aptos2.TxnBuilderTypes.RawTransaction);
      const secondarySignersAddressesV1 = (_a = transaction.secondarySignerAddresses) == null ? void 0 : _a.map(
        (address) => convertV2toV1(address, import_aptos2.TxnBuilderTypes.AccountAddress)
      );
      let rawTxn;
      if (asFeePayer) {
        const activeAccount = await this.account();
        const feePayerAddressV1 = import_aptos2.TxnBuilderTypes.AccountAddress.fromHex(activeAccount.address);
        rawTxn = new import_aptos2.TxnBuilderTypes.FeePayerRawTransaction(
          rawTxnV1,
          secondarySignersAddressesV1 ?? [],
          feePayerAddressV1
        );
      } else if (transaction.feePayerAddress) {
        const feePayerAddressV1 = convertV2toV1(transaction.feePayerAddress, import_aptos2.TxnBuilderTypes.AccountAddress);
        rawTxn = new import_aptos2.TxnBuilderTypes.FeePayerRawTransaction(
          rawTxnV1,
          secondarySignersAddressesV1 ?? [],
          feePayerAddressV1
        ...
        rawTxn = new import_aptos2.TxnBuilderTypes.MultiAgentRawTransaction(
          rawTxnV1,
          secondarySignersAddressesV1
        );
      } else {
      ...
      try {
        ...
        return convertV1toV2(accountAuthenticator, import_ts_sdk2.AccountAuthenticator);
      } catch (err) {
        if (isObjectPropsUnsupportedError(err)) {
